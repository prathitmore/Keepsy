...
class BackupManager(
    ...
    suspend fun exportBackupJson(): String = withContext(Dispatchers.IO) {
        KeepsyLogger.i("Exporting local backup...")
        ...
        if (imagesDir.exists()) {
            imagesDir.listFiles()?.forEach { file ->
                try {
                    ...
                    KeepsyLogger.e("Failed to read image for backup: ${file.name}", e)
                }
            }
        }
    ...
    suspend fun importBackupJson(jsonString: String): Boolean = withContext(Dispatchers.IO) {
        KeepsyLogger.i("Importing local backup...")
        try {
            val adapter = moshi.adapter(BackupContainer::class.java)
            ...
            for (imageItem in container.images) {
                try {
                    ...
                    KeepsyLogger.e("Failed to restore image: ${imageItem.fileName}", e)
                }
            }
            ...
            KeepsyLogger.i("Local backup restoration complete")
            true
        } catch (e: Exception) {
            KeepsyLogger.e("Backup restoration failed", e)
            false
        }
    }
}
